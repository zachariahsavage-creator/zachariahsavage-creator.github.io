...
function setupHireAvailabilityPill() {
  ...
  function scrollToContact() {
    ...
    const isInView = rect.bottom > 120 && rect.top < window.innerHeight * 0.6;
    if (isNearTop || isInView) return;
  ...
  // Mobile safeguard: a finger drag during scrolling can sometimes end up
  // firing a `click` on the fixed CTA. Track whether this was a "tap"
  // (small movement) before allowing the scroll.
  ...
  const TAP_SCROLL_PX = 24;

  function onActionPointerDown(e) {
    // Only consider primary/primary-touch interactions.
  ...
  actionBtn.addEventListener("click", (e) => {
    ...
    if (tapMoved || scrollDelta > TAP_SCROLL_PX) return;
    pendingTap = false;
  ...
  actionBtn.addEventListener("lostpointercapture", onActionPointerCancel);
