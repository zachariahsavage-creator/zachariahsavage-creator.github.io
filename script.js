...
const ITEM_WIDTH = 288; // 338 * 0.85; keep CSS + layout math in sync
...
function measureAndPosition(rowsState) {
  ...
  rowsState.forEach((rowState, rowIndex) => {
    ...
    if (originalCount > 0) {
      ...
      const sequenceWidth = baseCount * (ITEM_WIDTH + spacing);
      const baseItems = rowState.items.slice(0, baseCount);
      let sequenceOffset = 0;
      let lastRight =
        rowState.items[rowState.items.length - 1].x + ITEM_WIDTH;
      ...
      while (lastRight < targetRight) {
        ...
        for (let i = 0; i < baseCount && lastRight < targetRight; i++) {
          ...
          currentX += ITEM_WIDTH + spacing;
          lastRight = cloneState.x + ITEM_WIDTH;
        }
      }
    }
  });
...
function startAnimation(rowsState) {
  ...
  function step(now) {
    ...
    const viewportWidth = window.innerWidth;
    const wrapRightThreshold = viewportWidth + ITEM_WIDTH;
    const wrapLeftThreshold = -ITEM_WIDTH;
...
window.addEventListener("load", () => {
  ...
  // Hamburger menu: toggle PROFILE, CONTACT
  ...
  if (menuContainer && menuTrigger && menuContent) {
    ...
    // Click heading (GALLERY) to open/close menu
